...
<style>
    ...
    <h1 class="h3 mb-3 text-gray-800">Indeks IRH</h1>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 mt-2 font-weight-bold text-primary">List Indeks IRH</h6>
            <div class="float-right d-inline">
                <a href="{{ route('admin.index_hukum.irh.create') }}" class="btn btn-primary btn-sm">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-fixed table-condensed table-responsive table-striped" id="dataTable" width="100%" cellspacing="0" style="font-size: small;">
                    ...
                    <tbody>
                        @foreach($indexIrh as $row)
                        <tr>
                            <td style="text-align: center; width: 2%; vertical-align: middle;">
                                {{ $loop->iteration }}
                            </td>
                            <td style="width: 50%; vertical-align: middle;">{{ $row->irh_name }}</td>
                            <td style="width: 15%; text-align: center; vertical-align: middle;">{{ $row->irh_year }}</td>
                            <td style="width: 15%; text-align: center; vertical-align: middle;">{{ $row->irh_score }}</td>
                            <td style="width: 2%; text-align: center;">
                                @if($row->irh_file)
                                    <a href="#" data-href="{{ URL::to('storage/places/index_hukum/'.$row->irh_file) }}" data-toggle="modal" data-keyboard="false" data-backdrop="static" data-target="#viewFile" class="btn btn-sm btn-custom vFile">
                                        <span>
                            ...
                            <td style="text-align: center; width: auto;">
                                @php $irhID = Crypt::encrypt($row->id); @endphp
                                <a href="{{ URL::to('admin/index-hukum/irh/edit/'.$irhID) }}" class="btn btn-warning btn-sm">
                                    <i class="fas fa-edit"></i>
                                </a>
                                
                                <a href="#" data-toggle="modal" data-keyboard="false" data-backdrop="static" data-target="#confirm-delete" class="btn btn-danger btn-sm" data-href="{{ URL::to('admin/index-hukum/irh/delete/'.$row->id) }}">
