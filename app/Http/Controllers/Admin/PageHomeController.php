...
class PageHomeController extends Controller
{
    ...
    public function update3(Request $request)
    {
        if($request->hasFile('special_bg'))
        {
            ...
            Storage::putFileAs('public/places', $request->file('special_bg'), $final_name);
//            $request->file('special_bg')->move(public_path('uploads/'), $final_name);
        ...
        if($request->hasFile('special_video_bg'))
        {
            ...
            // Unlink old photo
            unlink(public_path('storage/places/'.$request->input('current_photo1')));

            // Uploading new photo
            $ext = $request->file('special_video_bg')->extension();
            $final_name = 'special_video_bg'.'.'.$ext;
            Storage::putFileAs('public/places', $request->file('special_video_bg'), $final_name);
//            $request->file('special_video_bg')->move(public_path('uploads/'), $final_name);
    ...
    public function update5(Request $request)
    {
        if($request->hasFile('testimonial_bg'))
        {
            ...
            Storage::putFileAs('public/places', $request->file('testimonial_bg'), $final_name);
//            $request->file('testimonial_bg')->move(public_path('uploads/'), $final_name);
    ...
    public function update8(Request $request)
    {
        if($request->hasFile('appointment_bg'))
        {
            ...
            Storage::putFileAs('public/places', $request->file('appointment_bg'), $final_name);
//            $request->file('appointment_bg')->move(public_path('uploads/'), $final_name);
    ...
    public function update10(Request $request)
    {
        if($request->hasFile('newsletter_bg'))
        {
            ...
            Storage::putFileAs('public/places', $request->file('newsletter_bg'), $final_name);
//            $request->file('newsletter_bg')->move(public_path('uploads/'), $final_name);
    ...
    public function update11(Request $request) {
        if($request->hasFile('artikel_bg'))
        {
            $request->validate([
                'artikel_bg' => 'image|mimes:jpeg,png,jpg,gif|max:2048'
            ]);
            ...
            $final_name = 'artikel_bg'.'.'.$ext;
            Storage::putFileAs('public/places', $request->file('artikel_bg'), $final_name);
//            $request->file('artikel_bg')->move(public_path('uploads/'), $final_name);

            $data['artikel_bg'] = $final_name;
        }

        $data['artikel_title'] = $request->input('artikel_title');
        $data['artikel_subtitle'] = $request->input('artikel_subtitle');
        $data['artikel_status'] = $request->input('artikel_status');

        PageHomeItem::where('id',1)->update($data);
        return redirect()->back()->with('success', 'Artikel Section is updated successfully!');
    }
    public function update12(Request $request) {
        if($request->hasFile('majalah_bg'))
        {
            $request->validate([
                'majalah_bg' => 'image|mimes:jpeg,png,jpg,gif|max:2048'
            ]);
            ...
            $final_name = 'majalah_bg'.'.'.$ext;
            Storage::putFileAs('public/places', $request->file('majalah_bg'), $final_name);
//            $request->file('majalah_bg')->move(public_path('uploads/'), $final_name);

            $data['majalah_bg'] = $final_name;
        }

        $data['majalah_title'] = $request->input('majalah_title');
        $data['majalah_subtitle'] = $request->input('majalah_subtitle');
        $data['majalah_status'] = $request->input('majalah_status');

        PageHomeItem::where('id',1)->update($data);
        return redirect()->back()->with('success', 'Majalah Section is updated successfully!');
    }
    
    public function update13(Request $request) {
        if($request->hasFile('peraturan_bg'))
        {
            $request->validate([
                'peraturan_bg' => 'image|mimes:jpeg,png,jpg,gif|max:2048'
            ]);
            ...
            $final_name = 'peraturan_bg'.'.'.$ext;
            Storage::putFileAs('public/places', $request->file('peraturan_bg'), $final_name);
//            $request->file('peraturan_bg')->move(public_path('uploads/'), $final_name);

            $data['peraturan_bg'] = $final_name;
        }

        $data['peraturan_title'] = $request->input('peraturan_title');
        $data['peraturan_subtitle'] = $request->input('peraturan_subtitle');
        $data['peraturan_status'] = $request->input('peraturan_status');
